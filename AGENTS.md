...
- **Two data sources**, chosen explicitly in the admin panel and read/written via [src/lib/admin-data.ts](src/lib/admin-data.ts):
  ...
  - Every write goes through `validateRawData()` first — per-collection Zod validation, then referential-integrity checks (relationship/grave `personId`, and every id in media's `personIds`, must reference a real person) that the schemas alone don't enforce. A media item can be tagged with multiple people (`personIds: string[]`, min 1) since one photo often shows several family members; the schema still accepts a legacy single `personId` string and normalizes it to `personIds` on read.
...
- **Private photos**: uploads go through Vercel Blob (`access: "private"`), never a public URL — served only via `/api/media/[...pathname]`, which checks `family-session` itself (defense in depth, not just relying on `src/proxy.ts`). Images are resized **at upload time** via Blob's `putImage()`, not via Next's `<Image>` on-demand optimizer — that optimizer fetches server-side without forwarding the browser's session cookie, so it would get a 401 from the private media route. Every `<Image>` rendering one of these URLs is `unoptimized` deliberately, not by oversight. Gallery media is resized to one 1600px variant. Avatars are **progressive**: each upload produces two separate blobs via `uploadAvatarImage()` in [src/lib/blob.ts](src/lib/blob.ts) — a 160px `small` variant for the tree view and relative cards (rendered at ~40px, so this covers retina with headroom), and a 480px `large` variant for the person detail hero (rendered at 120px). `Person.avatar` is `{small, large}`; the Zod schema also still accepts a legacy plain-string avatar (older data, one resolution reused for both) and normalizes it on read — no forced data migration. Real production avatars uploaded before this scheme shipped can still be multi-hundred-KB originals served at ~50px display size (confirmed via a Lighthouse audit: this was the single largest performance opportunity on the site, ~3.3 MB / ~950ms of avoidable LCP delay) — `pnpm migrate:avatars` (needs `.env.local` with real Blob credentials, i.e. `vercel env pull` first) finds every person whose `avatar.small === avatar.large` and re-encodes both variants via the same `putImage()` path `uploadAvatarImage()` uses, for either data source (`--source=local` default, or `--source=global-config`). Use `--dry-run` first. It never deletes the original oversized blob.
...
- **Demo mode**: `demoModeFlag` in [src/lib/flags.ts](src/lib/flags.ts) (Vercel Flags SDK — Edge Config first, `DEMO_MODE` env var fallback) switches locale to English, points `readLocalData()` at `src/data/demo/` (fictional people, `randomuser.me`/`picsum.photos` stock photos) instead of `src/data/` or Global Config, makes both gate stages accept any (or empty) input, and 404s `/admin/*` in `proxy.ts` — with `verifyAdminLogin` also refusing independently as defense in depth. The flags discovery endpoint (`src/app/.well-known/vercel/flags/route.ts`) is exempted from the gate in `proxy.ts` since it authenticates via `FLAGS_SECRET`, not a session cookie. See the README's "Demo mode" section for how to turn it on.
...
- `DEMO_MODE` — set to `true` to run this deployment as the public demo build (see "Demo mode" above). `EDGE_CONFIG` (auto-set once an Edge Config store is connected) and a `FLAGS_SECRET` (for Vercel's Flags Explorer) make the flag dashboard-togglable instead of requiring a redeploy.
